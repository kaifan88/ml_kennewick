...
RF1_feature_table = pd.DataFrame(columns = ['PBL_m', 'Sea_level_Pressure_Pa', 'Temp_K', 'WindSpeed_m_per_s',
       'WindDir_deg', 'RH_pct', 'Month', 'Hour', 'Weekday', 'past_O3_24', 'Date']) #dfPm.keys()[2:]) 
RF2_feature_table = pd.DataFrame(columns = ['PBL_m', 'Sea_level_Pressure_Pa', 'Temp_K', 'WindSpeed_m_per_s',
       'WindDir_deg', 'RH_pct', 'Month', 'Hour', 'Weekday', 'past_O3_24', 'Date']) #dfPm.keys()[2:])
for d in sorted(set(dfPm[(dfPm.index.year > 2017)].index.date)):
    ...
    new_train['obs_o3'] = Y
    new_train['pred_o3'] = model_RF.predict(X)
    new_train['diff_o3'] = abs(new_train['pred_o3']-new_train['obs_o3'])
    new_train=new_train[new_train['diff_o3']>5]
    X2 = np.array(new_train.drop(['obs_o3','pred_o3','diff_o3'],axis=1))
    Y2 = Y[new_train.index]
    model_RF2 = RandomForestRegressor(n_estimators=200, max_depth=7,random_state=137)
    model_RF2 = model_RF2.fit(X2, Y2)
    
    RF1_feature_table = RF1_feature_table.append(dict(zip(RF1_feature_table.columns, np.append(model_RF.feature_importances_,d))), ignore_index=True)
    RF2_feature_table = RF2_feature_table.append(dict(zip(RF2_feature_table.columns, np.append(model_RF2.feature_importances_,d))), ignore_index=True)
    RF1_feature_table.to_csv(
        'D:/Research/ML/cross_validation_aqs/ML2_time_RF1_feature.csv', index=False)
    RF2_feature_table.to_csv(
        'D:/Research/ML/cross_validation_aqs/ML2_time_RF2_feature.csv', index=False)
    #RF1_feature_table.to_csv('D:\Research\ML\ML2_time_RF1_feature_noAQI4.csv',index=False)
    #RF2_feature_table.to_csv('D:\Research\ML\ML2_time_RF2_feature_noAQI4.csv',index=False)
...
df2019_org.to_csv('D:/Research/ML/cross_validation_aqs/Kennewick_RF2_time_1719.csv')
df2018dailyO38hrmax.to_csv('D:/Research/ML/cross_validation_aqs/Kennewick_max_RF2_time_1719.csv')
#df2019_org.to_csv('D:\Research\ML\Kennewick_RF2_time_1719_noAQI4.csv')
#df2018dailyO38hrmax.to_csv('D:\Research\ML\Kennewick_max_RF2_time_1719_noAQI4.csv')
